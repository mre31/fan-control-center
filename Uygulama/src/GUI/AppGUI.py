...
class FanControlGUI(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        try:
            ...
            # Kaydedilen minimize to tray ayarını yükle
            self._minimize_to_tray = self._loadTraySettings()
            
            # Fan kontrolünü başlat
            self._fanControl = FanControl()
            
            # Create Translator
            self.translator = Translator()
            
            # GUI bileşenlerini yükle
            self._setup_ui()
            
        except Exception as e:
            raise Exception(f"GUI başlatılamadı: {str(e)}")

    def closeEvent(self, event):
        """Pencere kapatıldığında"""
        try:
            # Minimize to tray ayarı aktifse sistem tepsisine küçült
            if hasattr(self, '_minimize_to_tray') and self._minimize_to_tray:
                ...
                # Minimize to tray kapalıysa uygulamayı tamamen kapat
                self._onQuit()
                event.accept()
    ...
    def _setup_ui(self):
        try:
            ...
            logging.error(f"FanControlGUI initialization failed: {str(e)}\n{traceback.format_exc()}")
            raise
    ...
    def create_pages(self):
        ...
        self.tray_check.setChecked(self._minimize_to_tray)
    ...
    def _createTrayIcon(self):
        ...
        self.trayIcon.setIcon(self._app_icon)  # Önceden yüklenen ikonu kullan
        
        # Tray menüsü oluştur
        self.trayMenu = QtWidgets.QMenu()
        
        # Ana menü öğeleri
        showAction = QtGui.QAction(self.translator.get_text('show'), self.trayMenu)
        showAction.triggered.connect(self.show)
        
        # Profiller için alt menü
        profilesMenu = QtWidgets.QMenu(self.translator.get_text('profiles'), self.trayMenu)
        
        # Profil seçenekleri için action group
        profileGroup = QtGui.QActionGroup(profilesMenu)
        ...
        for profile in DEFAULT_PROFILES:
            translated = profile_translations.get(profile, profile)
            action = QtGui.QAction(translated, profilesMenu, checkable=True)
            action.setActionGroup(profileGroup)
            if profile == current_profile:
                action.setChecked(True)
            # userData olarak orijinal profil adını sakla
            action.setData(profile)
            action.triggered.connect(lambda checked, p=profile: self._onProfileMenuSelect(p))
            profilesMenu.addAction(action)
        
        quitAction = QtGui.QAction(self.translator.get_text('exit'), self.trayMenu)
        quitAction.triggered.connect(self._onQuit)
        
        # Menüyü oluştur
        self.trayMenu.addAction(showAction)
        self.trayMenu.addMenu(profilesMenu)
    ...
    def _onProfileMenuSelect(self, profile_name: str):
        """Tray menüsünden profil seçildiğinde"""
        index = self._profileCombo.findData(profile_name)
        if index >= 0:
        ...
        try:
            if hasattr(self, '_fanControl'):
                ...
                settings = {
                    ...
                    'last_profile': self._fanControl.profile_manager.current_profile if hasattr(self._fanControl, 'profile_manager') else None
                }
    ...
    def _loadTraySettings(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(settings_path):
                with open(settings_path, 'r') as f:
                    settings = json.load(f)
                    return settings.get('minimize_to_tray', True)  # Varsayılan olarak aktif
        except:
            pass
        return True  # Dosya/ayar yoksa varsayılan olarak aktif
